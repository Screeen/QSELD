...
def temporal_block(inp, num_filters_temporal, dropout=0, recurrent_type='gru', data_in=(),
    ...
    if recurrent_type == 'gru':
        ...
        # 1D convolution
        h = Conv1D(filters=nb_1x1_filters, kernel_size=1, padding='same', data_format=tcn_data_format)(h)
        h = Activation('relu')(h)
...
def get_model(data_in, data_out, dropout_rate, nb_cnn2d_filt, pool_size,
              rnn_size, fnn_size, classification_mode, weights, recurrent_type='gru', data_format='channels_first'):
    ...
    spec_cnn = temporal_block(spec_cnn, num_filters_temporal=rnn_size, dropout=dropout_rate,
                              recurrent_type='TCN', data_in=data_in, input_data_format=data_format)
...
#
#     spec_start = Input(shape=(inp_shape[-3], inp_shape[-2], inp_shape[-1]))
#     print(f"K.int_shape(spec_start) {K.int_shape(spec_start)}")
#     spec_cnn = spec_start
#     for i, convCnt in enumerate(pool_size):
#         spec_cnn = Conv2D(filters=nb_cnn2d_filt, kernel_size=(3, 3), padding='same')(spec_cnn)
