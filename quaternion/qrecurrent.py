...
from quaternion import qinitializer
...
class QuaternionGRUCell(Layer):
    ...
    def build(self, input_shape):
        ...
        kern_init = qinitializer.qdense_init(init_shape, self.init_criterion)
    ...
    def call(self, inputs, states, training=None):
        ...
        if self.implementation == 1:
            ...
            cat_kernels_4_quaternion = K.concatenate(
                [cat_kernels_4_r, cat_kernels_4_i, cat_kernels_4_j, cat_kernels_4_k], axis=0)
...
class QuaternionRNNCell(Layer):
    ...
    def call(self, inputs, states):
        ...
        self.r = self.recurrent_kernel[:, :self.q_units]
        self.i = self.recurrent_kernel[:, self.q_units:self.q_units * 2]
        self.j = self.recurrent_kernel[:, self.q_units * 2:self.q_units * 3]
        self.k = self.recurrent_kernel[:, self.q_units * 3:]

        cat_kernels_4_r = K.concatenate([self.r, -self.i, -self.j, -self.k], axis=-1)
        cat_kernels_4_i = K.concatenate([self.i, self.r, -self.k, self.j], axis=-1)
        cat_kernels_4_j = K.concatenate([self.j, self.k, self.r, -self.i], axis=-1)
        cat_kernels_4_k = K.concatenate([self.k, -self.j, self.i, self.r], axis=-1)
        cat_kernels_4_quaternion = K.concatenate([cat_kernels_4_r, cat_kernels_4_i, cat_kernels_4_j, cat_kernels_4_k],
                                                 axis=0)

        w = K.dot(prev_output, cat_kernels_4_quaternion)

        r_input = w[:, :self.units]
        i_input = w[:, self.units:self.units * 2]
        j_input = w[:, self.units * 2:self.units * 3]
        k_input = w[:, self.units * 3:]

        w = K.concatenate([r_input, i_input, j_input, k_input], axis=-1)
        output = h + w
...
class QuaternionRNN(RNN):
    ...
    def __init__(self, units,
        ...
        cell = QuaternionRNNCell(units,
                                 ...
                                 dropout=dropout,
                                 recurrent_dropout=recurrent_dropout)
        super(QuaternionRNN, self).__init__(cell,
                                            return_sequences=return_sequences,
                                            return_state=return_state,
                                            go_backwards=go_backwards,
                                            stateful=stateful,
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None):
        self.cell._dropout_mask = None
        self.cell._recurrent_dropout_mask = None
        return super(QuaternionRNN, self).call(inputs,
                                               mask=mask,
                                               training=training,
                                               initial_state=initial_state)
    ...
    def get_config(self):
        config = {'units': self.units}
        base_config = super(QuaternionRNN, self).get_config()
        del base_config['cell']
        return dict(list(base_config.items()) + list(config.items()))
