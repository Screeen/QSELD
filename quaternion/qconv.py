...
from tensorflow.keras.layers import Layer, Lambda, InputSpec, Convolution1D, Convolution2D, add, multiply, Activation, \
    Input, concatenate
# from keras.backend import common
from keras.models import Model
import numpy as np
from quaternion.qinitializer import *
import sys
...
class QuaternionConv(Layer):
    ...
    def __init__(self, rank,
        ...
        # print("kernel size normalized: ", self.kernel_size)
    ...
    def call(self, inputs, **kwargs):
        ...
        filters_times_2 = self.filters * 2
        filters_times_3 = self.filters * 3

        f_r = self.kernel[..., :self.filters]
        f_i = self.kernel[..., self.filters:filters_times_2]
        f_j = self.kernel[..., filters_times_2:filters_times_3]
        f_k = self.kernel[..., filters_times_3:]

        if self.kernel_shape[-1] < filters_times_3:
            raise ValueError("Kernel shape too small: {}. index3: {}", self.kernel_shape, filters_times_3)
